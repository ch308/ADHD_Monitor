...
class CloudService {
  ...
  Future<int?> createChild(String nickname) async {
    try {
      final r = await http
          .post(
          ...
          .timeout(timeout);
      if (r.statusCode != 200) {
        debugPrint('CloudService: create child ${r.statusCode}: ${r.body}');
        return null;
      }
      final data = json.decode(r.body) as Map<String, dynamic>;
      final raw = data['child_id'];
      if (raw is num) return raw.toInt();
      return int.tryParse(raw?.toString() ?? '');
    } catch (e) {
      debugPrint('CloudService: create child error $e');
      return null;
    }
  }
  ...
  Future<bool> saveTeacherAlertEvent(TeacherAlertEvent event) async {
    try {
      final body = event.toJson();
      body['teacher_band_notified'] = event.teacherBandNotified;
      final r = await http
          .post(
            Uri.parse('${_base()}/teacher/alert_events'),
            headers: _authHeaders(),
            body: json.encode(body),
          )
          .timeout(timeout);
      ...
      debugPrint('CloudService: save teacher alert ${r.statusCode}: ${r.body}');
      return false;
    } catch (e) {
      debugPrint('CloudService: save teacher alert error $e');
      return false;
    }
  }

  Future<List<TeacherAlertEvent>> fetchTeacherAlertEvents({
    int limit = 80,
  }) async {
    try {
      final r = await http
          .get(
            Uri.parse('${_base()}/teacher/alert_events?limit=$limit'),
            headers: _authHeaders(jsonBody: false),
          )
          .timeout(timeout);
      if (r.statusCode != 200) {
      ...
      return events
          .whereType<Map>()
          .map((item) => TeacherAlertEvent.fromJson(
