...
def init_db():
    ...
    cursor.execute(
        """
        CREATE TABLE IF NOT EXISTS teacher_alert_events (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            note TEXT,
            teacher_band_notified INTEGER NOT NULL DEFAULT 0,
            created_at TEXT NOT NULL,
            FOREIGN KEY (user_id) REFERENCES users(id)
        )
        """
...
@app.route("/teacher/alert_events", methods=["POST", "GET"])
def teacher_alert_events():
    user = _get_request_user()
    ...
    if request.method == "POST":
        ...
        student_name = (
            data.get("studentNameSnapshot")
            or data.get("student_name_snapshot")
            or "学生"
        ).strip()
        ...
        if not student_id or not event_type:
            return jsonify({
                "status": "error",
                "message": "student_id and event_type are required",
            }), 400
        ...
        created_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        teacher_band_notified = 1 if (
            data.get("teacherBandNotified") or data.get("teacher_band_notified")
        ) else 0
        ...
        try:
            ...
            cursor.execute(
                """
                INSERT INTO teacher_alert_events
                (client_event_id, user_id, student_id, student_name_snapshot,
                 event_type, bpm, threshold_bpm, started_at, note,
                 teacher_band_notified, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    str(data.get("id") or data.get("client_event_id") or "").strip(),
                    user["id"] if user else None,
                    student_id,
                    student_name,
                    event_type,
                    _optional_int(data.get("bpm")),
                    _optional_int(data.get("thresholdBpm") or data.get("threshold_bpm")),
                    started_at,
                    (data.get("note") or "").strip(),
                    teacher_band_notified,
                    created_at,
                ),
            )
            ...
            app.logger.exception("teacher alert insert failed")
            return jsonify({"status": "error", "message": str(e)}), 500
    ...
    try:
        ...
        events = [
            {
                "id": str(row[0]),
                "client_event_id": row[1],
                "student_id": row[2],
                "student_name_snapshot": row[3],
                "event_type": row[4],
                "bpm": row[5],
                "threshold_bpm": row[6],
                "started_at": row[7],
                "note": row[8],
                "teacher_band_notified": bool(row[9]),
                "created_at": row[10],
                "uploaded": True,
            }
            for row in rows
        ...
        app.logger.exception("teacher alert query failed")
        return jsonify({"status": "error", "message": str(e)}), 500
