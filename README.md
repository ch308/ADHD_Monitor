...
| `_stressAlertTriggered` | stress 阈值触发去抖标志 |
...
2. **手环 stress 通道**：`_maybeTriggerStressAlert(stress)`，阈值 `_stressAlertThreshold = 60`：
...
stress 通道独立去抖：`stress < 阈值-5` 才允许 `_stressAlertTriggered` 复位。
