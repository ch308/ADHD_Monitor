...
# Slim fork: only ship firmware for this SKU (other board dirs may remain in tree but are ignored).
_SLIM_BOARD_ALLOW = frozenset({"xingzhi-cube-1.54tft-wifi"})
...
def _collect_variants(config_filename: str = "config.json") -> List[Dict[str, str]]:
    ...
    for cfg_path in _BOARDS_DIR.rglob(config_filename):
        ...
        if board not in _SLIM_BOARD_ALLOW:
            continue
