...
void LcdDisplay::ShowFullscreenImage(const lv_image_dsc_t* image) {
    DisplayLockGuard lock(this);
    if (image == nullptr) {
        return;
    }
    ...
    if (action_image_ == nullptr) {
        action_image_ = lv_image_create(screen);
    ...
    // no-op for 240x240, but keeps things correct if a different size is fed.
    if (image->header.w > 0) {
        lv_image_set_scale(action_image_, 256 * width_ / image->header.w);
    }
    lv_obj_move_foreground(action_image_);
    lv_obj_remove_flag(action_image_, LV_OBJ_FLAG_HIDDEN);
}
