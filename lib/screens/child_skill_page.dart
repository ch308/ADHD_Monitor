...
class _ChildSkillPageState extends State<ChildSkillPage>
  ...
  Future<void> _initCuteVoice() async {
  ...
  Future<void> _speakCute(String text) async {
  ...
  Future<void> _ask(String rawQuestion) async {
    ...
    setState(() {
      _asking = false;
      _messages.add(_SkillMessage.bot(answer));
    });
    unawaited(_speakCute(answer));
  }
...
class _ImageChildAvatar extends StatelessWidget {
  const _ImageChildAvatar({
    required this.skill,
    required this.animation,
    required this.waving,
    required this.speaking,
  ...
  String get _assetPath {
    final gender = skill.profile.gender?.trim() ?? '';
    if (gender.contains('女')) {
      return 'assets/images/child_girl_character.png';
    }
    return 'assets/images/child_boy_character.png';
  }
      ...
      child: SizedBox(
        ...
        child: Image.asset(
          ...
          errorBuilder: (context, error, stackTrace) {
            return const _ImageFallbackAvatar();
          },
        ),
      ),
    );
...
class _ImageFallbackAvatar extends StatelessWidget {
  const _ImageFallbackAvatar();
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const Icon(Icons.child_care_rounded, color: _sage, size: 82),
    );
  }
}
